...
class LocalAudioManager(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    fun playRequired(resourceName: String): Boolean {
        ...
        // The faulty letter-sound recordings are temporarily routed to the
        // separate Letter Name path. The Letter Sound button remains present
        // in the UI and keeps the same resource name so real phonics audio can
        // replace this fallback later without changing the screen.
        Regex("en_letter_(\\d{2})_sound").matchEntire(resourceName)?.let {
            val index = it.groupValues[1].toInt() - 1
            if (index in 0..25) {
                // Known faulty phonics entries: E, F, I, L, N, O, P, Q, R,
                // S, U, V, X and Z. Until proper phonics recordings are added,
                // these must say the actual English letter name instead of a
                // wrong/repeated phoneme. All other existing sounds remain
                // untouched.
                val faulty = setOf(4, 5, 8, 11, 13, 14, 15, 16, 17, 18, 20, 21, 23, 25)
                if (index in faulty) return speakEnglishLetterName(index)
                return speakEnglishLetterSound(index)
            }
        }
    ...
    private fun speakEnglishLetterName(index: Int): Boolean {
        if (!enabled || !ttsReady || index !in 0..25) return false
        return speakOffline(('A'..'Z').elementAt(index).toString(), "en")
    }

    /**
     * English A-Z Letter Sound only.
     *
     * This intentionally does NOT speak the letter character for the letters
     * that still have working phonics cues. Android TTS reads a standalone
     * character as its letter name. These short cues remain the temporary
     * offline path for the good entries until dedicated phonics recordings are
     * available.
     */
