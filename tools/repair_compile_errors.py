...
# Normalize repairs from earlier bootstrap runs so repeated workflow runs are idempotent.
normalize_text('EnglishLettersScreen.kt', [
    ('import androidx.compose.material3.MaterialTheme\nimport androidx.compose.material3.MaterialTheme\n',
     'import androidx.compose.material3.MaterialTheme\n'),
])

normalize_text('LettersScreen.kt', [
    ('ArabicArabicLetterForm', 'ArabicLetterForm'),
])
...
# LettersScreen: keep its own Arabic form enum and normalize any malformed references.
def letters_fix(s):
    ...
    for old, new in [
        ('mutableStateOf(LetterForm.INITIAL)', 'mutableStateOf(ArabicLetterForm.INITIAL)'),
...
# ReadingScreen: normalize its private form enum, pointer API and explicit audio types.
def reading_fix(s):
    ...
    s = s.replace('audio.playSequence(ids.map { id -> "ar_letter_%02d_sound".format(id) })',
                  'val clips: List<String> = ids.map { id: Int -> "ar_letter_%02d_sound".format(id) }\n                        audio.playSequence(clips)')
    s = s.replace('private fun displayTarget(mode: ReadingMode, index: Int, form: LetterForm): String',
                  'private fun displayTarget(mode: ReadingMode, index: Int, form: ReadingLetterForm): String')
    return s
...
# WritingStrokeLessonScreen: repair prohibited vararg Offset and positional modifiers without changing lesson content.
def stroke_fix(s):
